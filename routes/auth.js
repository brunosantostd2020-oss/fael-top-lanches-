...
function requireGerenciamento(req, res, next) {
  const auth = req.headers.authorization;
  if (!auth || !auth.startsWith('Bearer ')) {
    return res.status(401).json({ error: 'Token necessário' });
  }
  try {
    const payload = jwt.verify(auth.split(' ')[1], JWT_SECRET);
    if (!payload.gerenciamento) return res.status(403).json({ error: 'Acesso negado' });
    req.gerenciamento = payload;
    next();
  } catch {
    res.status(401).json({ error: 'Token inválido ou expirado' });
  }
...
// POST /api/auth/login-gerenciamento - Gerenciamento
router.post('/login-gerenciamento', async (req, res) => {
  const { password } = req.body;
  if (!password) return res.status(400).json({ error: 'Senha obrigatória' });
  try {
    const result = await pool.query("SELECT value FROM settings WHERE key = 'gerenciamento_password'");
    if (!result.rows.length || !bcrypt.compareSync(password, result.rows[0].value)) {
      return res.status(401).json({ error: 'Senha incorreta' });
    }
    const token = jwt.sign({ gerenciamento: true }, JWT_SECRET, { expiresIn: '12h' });
    res.json({ token, expiresIn: '12h' });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
// POST /api/auth/change-password-gerenciamento
router.post('/change-password-gerenciamento', requireGerenciamento, async (req, res) => {
  const { current_password, new_password } = req.body;
  if (!current_password || !new_password) return res.status(400).json({ error: 'Campos obrigatórios' });
  if (new_password.length < 4) return res.status(400).json({ error: 'Mínimo 4 caracteres' });
  try {
    ...
    const hash = bcrypt.hashSync(new_password, 10);
    await pool.query(
      "INSERT INTO settings (key, value) VALUES ('gerenciamento_password', $1) ON CONFLICT (key) DO UPDATE SET value=$1",
      [hash]
    );
    res.json({ success: true });
  } catch (e) {
...
// GET /api/auth/verify-gerenciamento
router.get('/verify-gerenciamento', requireGerenciamento, (req, res) => {
  res.json({ valid: true });
});

module.exports = { router, requireAdmin, requireGerenciamento };
